...
list<string> listaTiposDecFuncao;
list<string> listaTiposChamadaFuncao;

int quantidade_decFuncao = 0;
int quantidade_chamadaFuncao = 0;
...
S			: ESC_GLOBAL DECLARACOES ';' P
			...
			| ESC_GLOBAL DECLARACOES ';' FUNCAO P
			{
				declaracoes();
				$$.traducao = $2.traducao + "\n" + $4.traducao + "\n\n" + $5.traducao;
			}
...
DEC_FUNCAO : DECLARACAO ',' DEC_FUNCAO
			{
				listaTiposDecFuncao.push_front($1.tipo);

				$$.traducao = $1.dec_funcao + ", " + $3.traducao;
			}
			| DECLARACAO
			{
				listaTiposDecFuncao.push_front($1.tipo);

				$$.traducao = $1.dec_funcao;
			}
			;
...
FUNCAO		: TIPO TK_ID '(' DEC_FUNCAO ')' BLOCO 
			{
				...
				int count = 0;
				for(i = listaArgumentos.begin(); i != listaArgumentos.end(); i++) {
					cout << "### " << count << " " << *i << endl;
					count++;
				}*/

				// cout << "##### " << listaArgumentos.size() << endl;

				STRINGMAP* mapa = pilhaDeMapas.front();

				(*mapa)[$2.label].label = $2.label;
				(*mapa)[$2.label].tipo = $1.tipo;
				...
				if ($$.tipo == "void") {
					if ($6.traducao.find("return") != -1) {
						yyerror("aviso: ‘return’ com valor, em função retornando void");
					}
				} else {
					$$.traducao = $1.label + " " + $2.label + "(" + $4.traducao + ") {\n" + $6.traducao + "}";
				}

				listaTiposDecFuncao.clear();
			}
			;
...
CHAMADA_FUNCAO	: TK_ID '(' CHAMADA_TK_FUNCAO ')'
				{
					/*list<string>::iterator i;
					int count = 0;
					for(i = listaTiposChamadaFuncao.begin(); i != listaTiposChamadaFuncao.end(); i++) {
						cout << "### " << count << " " << *i << endl;
						count++;
					}*/
					...
					if (listaTiposDecFuncao.size() != listaTiposChamadaFuncao.size()) {
						yyerror("Quantidade de argumentos não confere.");
					}
					else if (listaTiposDecFuncao != listaTiposChamadaFuncao) {
						yyerror("Tipos das variáveis não conferem.");
					}
					else {
						$$.traducao = "\t" + $1.label + "(" + $3.traducao + ");";
					}

					listaTiposChamadaFuncao.clear();
				}
				;
...
CHAMADA_TK_FUNCAO	: TK_ID ',' CHAMADA_TK_FUNCAO
					{
						...
						listaTiposChamadaFuncao.push_front($$.tipo);
